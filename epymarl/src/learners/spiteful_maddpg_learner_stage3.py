...
class SpitefulMADDPGLearner_stage3:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Target networks are time-delayed copies of their original networks.
        ...
        target_actions = target_actions.view(batch_size, -1, 1, self.n_agents * self.n_actions).expand(-1, -1,
                                                                                                       self.n_agents,
                                                                                                       -1)
        ...
        # Inference of other agents' policies. C.f. equation 8 in MADDPG paper.
        targets = rewards.reshape(-1, 1) + self.args.gamma * (1 - terminated.reshape(-1, 1)) * target_vals.reshape(-1,
                                                                                                                   1).detach()
        ...
        - critic has a general overview, so he is rational
        - an agent does not have to be rational and can have spite
        
        q prior to reshaping is [32, 500, 2, 1] - we have 2 agents
        q after reshape as well as targets is flat: [32000, 1]
        ...
        #as proposed in our project proposal we need a "self motivation" alpha * avg_rewards
        #and also "harming others" beta * (harm_other - self_harm)
        motivation = 0.25 * (avg_q - avg_rewards).detach().cpu()

        #to assess harm dealt to others and self harm we propose a setting, where an agent imagines how
        #bad his actions would hurt him and interpolates it to the other agent
        #an agent randomly corrupts his actions
        for g in range(self.n_agents):
            nbatches = actions[:, :, g, :].shape[0]
            ncorrupted = int(nbatches * np.random.uniform(0, 1))
            if ncorrupted == 0:
                continue
            else:
                corrupt_batches = np.random.choice(range(nbatches), ncorrupted, replace=False)
                for c in corrupt_batches:
                    actions[c, :, g, :] = th.Tensor(np.random.permutation(actions[c, :, g, :].detach().cpu().numpy()))
        ...
        qq = self.critic(inputs[:, :-1], new_actions)
        avg_qq = th.mean(qq, dim=[0, 1, 3])
        drop_in_q = (avg_q - avg_qq).detach().cpu()
        drop_in_target = (avg_score - avg_qq).detach().cpu()

        scalar = th.FloatTensor([0])
        grieve = 0.5 * (drop_in_target - th.max(drop_in_q, scalar.expand_as(drop_in_q)))

        all_agents = list(range(self.n_agents))

        griefers = grieve > motivation
        griefers = [i for i, x in enumerate(griefers) if x]
        grieve_factor = th.tanh(drop_in_target - th.max(drop_in_q, scalar.expand_as(drop_in_q))).numpy()
        grieve_factor = np.where(grieve_factor < 0., 0., grieve_factor)

        for g in range(self.n_agents):
            if motivation[g] > grieve[g]:
                q[:, :, g, :] = q[:, :, g, :] + motivation[g]
            else:
                nbatches = actions[:, :, g, :].shape[0]
                ncorrupted = int(nbatches * np.random.uniform(0, 1))
                corrupt_batches = np.random.choice(range(nbatches), ncorrupted, replace=False)
                victim = np.random.choice(list(set(all_agents) - set([g])))
                for c in corrupt_batches:
                    q[c, :, g, :] = q[c, :, g, :] - drop_in_q[g]
                    q[c, :, victim, :] = q[c, :, victim, :] - drop_in_target[g]

        agents_q = th.mean(q, dim=[0, 1, 3])
        agents_q = agents_q.detach().cpu().numpy()
        ...
        if self.args.target_update_interval_or_tau > 1 and (
                episode_num - self.last_target_update_episode) / self.args.target_update_interval_or_tau >= 1.0:
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("agents_q", agents_q, t_env)
            self.logger.log_stat("griefers", griefers, t_env)
            self.logger.log_stat("grieve_factor", grieve_factor, t_env)
