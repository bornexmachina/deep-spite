...
class SpitefulMADDPGLearner_stage2:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        griefers = avg_score > avg_q * 0.75
        griefers = [i for i, x in enumerate(griefers) if x]

        all_agents = list(range(self.n_agents))

        grieve_factor = th.tanh(th.clamp((avg_score - avg_q) / avg_q, min=0)).detach().cpu().numpy()
        grieve_factor = np.where(grieve_factor < 0., 0., grieve_factor)

        if len(griefers) > 0:
            for g in griefers:
                nbatches = actions[:, :, g, :].shape[0]
                ncorrupted = (nbatches * grieve_factor[g]).astype(int)
                if ncorrupted == 0:
                    continue
                else:
                    corrupt_batches = np.random.choice(range(nbatches), ncorrupted, replace=False)

                    #trade own q value for damage in neighbors q
                    #start with trading 10% of own q to randomly harm someone else for 5%
                    #so in general it should be something like 2 : 1 self to other harm
                    selfharm = avg_rewards[g].detach().cpu().numpy() * 0.1
                    otherharm = selfharm / 2

                    #get a list of agents other than the griefer and
                    #pick randomly a victim to harm
                    #we assume that a griefer does not care, whether the others are griefing as well or not
                    victim = np.random.choice(list(set(all_agents) - set([g])))
                    for c in corrupt_batches:
                        q[c, :, g, :] = q[c, :, g, :] - selfharm
                        q[c, :, victim, :] = q[c, :, victim, :] - otherharm
